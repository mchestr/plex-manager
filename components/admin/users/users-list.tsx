...
import { UsersFilter as UsersFilterComponent, UsersFilter } from "./users-filter"
...
export function UsersList({ users, currentYear }: UsersListProps) {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter((user) => {
      ...
      if (filter.plexAccess === "yes" && user.hasPlexAccess !== true) {
        return false
      }
      if (filter.plexAccess === "no" && user.hasPlexAccess !== false) {
        return false
      }
      if (filter.plexAccess === "unknown" && user.hasPlexAccess !== null) {
        return false
      }
      ...
      // Filter by role
      if (filter.role === "admin" && !user.isAdmin) {
        return false
      }
      if (filter.role === "user" && user.isAdmin) {
        return false
      }
  ...
  return (
    <>
      <UsersFilterComponent onFilterChange={setFilter} defaultFilter={filter} />
